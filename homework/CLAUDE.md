...
`<Id>`/`<id>` is the problem's identifier (e.g. `P0`), matched to the program's file/class name. Don't explain what `<`/`>` redirection means in the statement — that's covered once, for students, in `course-docs/std-input-output` (built with `make std-input-output`), which also has the worked Problema P0 example this convention is based on.
